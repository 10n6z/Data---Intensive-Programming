...
val videogamesDF: DataFrame = spark.read
  .option("header", "true")
  .option("sep", "|")                  
...
val filteredSalesDF: DataFrame = videogamesDF
  .filter(year(col("release_date")) >= 2006 && year(col("release_date")) <= 2015)
  .groupBy("publisher")
  .agg(sum("total_sales").alias("sum"))

val bestNAPublisher: String = filteredSalesDF.orderBy(desc("sum")).first().getString(0)
val nullSalesCountDF: DataFrame = videogamesDF
  .filter(year(col("release_date")) >= 2006 && year(col("release_date")) <= 2015)
  .groupBy("publisher")
  .agg(count(when(col("na_sales").isNull, 1)).alias("null_na_sales_count"))

val titlesWithMissingSalesData: Long = nullSalesCountDF.filter(col("publisher") === bestNAPublisher).select("null_na_sales_count").head().getLong(0)

val bestNAPublisherSales: DataFrame = videogamesDF
  .filter(col("publisher") === "Activision" && year(col("release_date")) >= 2006 && year(col("release_date")) <= 2015)
...
val eventnodropDF: DataFrame = spark.read
  .option("header", "true")                  
  .option("inferSchema", "true")  
  .format("parquet")                 
  .load("abfss://shared@tunics320f2024gen2.dfs.core.windows.net/assignment/football/events.parquet")
val eventDF = eventnodropDF.drop("startPosition","endPosition","eventTime")
eventDF.show()
...
val matchDF: DataFrame = eventDF
  .filter(array_contains(col("tags"), "Goal"))
  .filter(col("event") === "Shot")
  .groupBy("matchId", "competition", "season", "homeTeam", "awayTeam")
  .agg(
    sum(when(col("eventTeam") === col("homeTeam"), 1).otherwise(0)).alias("homeTeamGoals"),
    sum(when(col("eventTeam") === col("awayTeam"), 1).otherwise(0)).alias("awayTeamGoals")
  )
  .select(
...
val combinedDF = homeDF.union(awayDF)

val seasonDF = combinedDF
  ...
  // .filter(col("competition") === "French Ligue 1")
  // .filter(col("team") === "Lille")
...
val englandDF: DataFrame = seasonDF
  ...
  .drop(col("competition"), col("season"))
  .groupBy(col("team"))
  .agg(
    sum(col("games")).alias("Pld"),
    sum(col("wins")).alias("W"),
    sum(col("draws")).alias("D"),
    sum(col("losses")).alias("L"),
    sum(col("goalsScored")).alias("GF"),
    sum(col("goalsConceded")).alias("GA"),
    sum(col("goalsScored") - col("goalsConceded")).alias("GD"),
    sum(col("points")).alias("Pts")
  )
  .select(
    col("team").alias("Team"),
    col("Pld"),
  ...
  .withColumn(
    "GD",
    when(col("GD") > 0, concat(lit("+"), col("GD")))
    .otherwise(col("GD").cast("string"))
  )
  .withColumn("Pos", dense_rank().over(windowSpec))
  .select(
    col("Pos"),
    col("Team"),
    col("Pld"),
...
val matchPassDF: DataFrame = eventDF
  ...
  .agg(
    count(col("event")).alias("totalPasses"),
    count(when(array_contains(col("tags"), "Accurate"), 1).otherwise(null)).alias("successfulPasses")
  )
  .select(
    col("matchId"),
    col("competition"),
...
val lowestPassSuccessRatioDF: DataFrame = matchPassDF
  .filter(col("season") === "2017-2018")
  .withColumn("passRatio", col("successfulPasses") / col("totalPasses"))
  .groupBy(col("team"), col("competition"))
  .agg(
    avg(col("passRatio")).alias("passSuccessRatio")
  )
  .groupBy(col("competition"))
  .agg(
    min(col("passSuccessRatio")).alias("passSuccessRatio")
  )
  .join(
    matchPassDF
      .filter(col("season") === "2017-2018")
    ...
    Seq("competition", "passSuccessRatio")
  )
  .select(
    "competition",
    "team",
  ...
  .orderBy(col("passSuccessRatio").asc)
...
val bestDF: DataFrame = seasonDF 
  .withColumn("GD", col("goalsScored") - col("goalsConceded")) 
  .join(passSuccessRatioDF, Seq("competition", "team"), "left")
  .select(
    col("team").alias("Team"),
    ...
    round((col("points")/col("games")), 2).alias("Avg"),
...
def toPixels(data: Array[Byte], channels: Int): Array[String] = {
    data
        .grouped(channels)
        .map(dataBytes =>
            dataBytes.map(byte => f"${byte & 0xff}%02X").mkString("")
        )
        .toArray
}
...
val metadatadf = spark.read
  .json("abfss://shared@tunics320f2024gen2.dfs.core.windows.net/assignment/openmoji/metadata")

// UDF to convert binary data to array of hex pixel strings
val toPixelsUDF = udf((data: Array[Byte]) => toPixels(data, 4))
// UDF to determine the color name from a hex pixel string
val toColorNameUDF = udf((hexString: String) => toColorName(hexString))

val imagedfWithHexcode = imagedf.select(
    col("image.origin").as("origin"),
    col("image.width").as("width"),
    col("image.height").as("height"),
...
val combinedDF = metadatadf.join(imagedfWithHexcode, Seq("hexcode"), "inner")

val colorDF = combinedDF
...
val coloredPixelCountsDF = colorDF
  .groupBy("annotation")
  .agg(
    sum(when(col("color_name") =!= "None", 1).otherwise(0)).as("non_transparent_count"),
    count("*").as("total_pixel_count")
  )
...
val mostColoredPixels = coloredPixelCountsDF
  .orderBy(desc("non_transparent_count"))
  .limit(4)
  .select("annotation").as[String]
  .collect()
...
val leastColoredPixels = coloredPixelCountsDF
...
val finlandFlagDF = combinedDF
  .filter(col("annotation") === "flag: Finland")

val finlandPixelDF = finlandFlagDF
  .withColumn("pixels", toPixelsUDF(col("data")))
  .withColumn("colors", explode(col("pixels")))
  .withColumn("color_name", toColorNameUDF(col("colors")))

val nonTransparentDF = finlandPixelDF
  .filter(col("color_name") =!= "None")

val colorCountsDF = nonTransparentDF
  .groupBy("color_name")
  .agg(count("*").as("count"))
  .orderBy(desc("count"))

val totalPixels = nonTransparentDF.count()

val topColorsDF = colorCountsDF
  .limit(3)
...
val finnishFlagColors: Array[String] = topColorsDF
  .select("color_name").as[String]
  .collect()

// The percentages of the colored pixels for each common color in the Finnish flag image:
val finnishColorShares: Array[Double] = topColorsDF
  .select("percentage").as[Double]
  .collect()

println("The colors and their percentage shares in the image for the Finnish flag:")
finnishFlagColors
  .zip(finnishColorShares)
  .foreach({case (color, share) => println(s"- color: ${color}, share: ${share}")})
println("============================================================")

val topColorsPerImageDF = combinedDF
  .withColumn("pixels", toPixelsUDF(col("data")))
  .withColumn("colors", explode(col("pixels")))
  .withColumn("color_name", toColorNameUDF(col("colors")))
  .filter(col("color_name") =!= "None")
  ...
  .groupBy("annotation")
  .agg(
    collect_list(struct(col("rank"), col("color_name"))).as("top_colors")
  )
  .withColumn("sorted_colors", sort_array(col("top_colors")))
  .withColumn("ordered_colors", expr("transform(sorted_colors, x -> x.color_name)"))

val targetColors = Array("Blue", "Yellow", "Black")
val matchingImagesDF = topColorsPerImageDF.filter(
  col("ordered_colors") === lit(targetColors)
)
// The number of images that have their most common three colors as, Blue-Yellow-Black, in that exact order:
val matchingCount = matchingImagesDF.count()

println(s"The number of images that have, Blue-Yellow-Black, as the most common colors: ${matchingCount}")
println("============================================================")

// COMMAND ----------

val redColorDF = combinedDF
  .withColumn("pixels", toPixelsUDF(col("data"))) 
  .withColumn("colors", explode(col("pixels"))) 
  .withColumn("color_name", toColorNameUDF(col("colors"))) 
  .filter(col("color_name") === "Red" && col("group") === "activities") 

val redPixelCountsDF = redColorDF
  .groupBy("annotation") 
  .agg(count("*").as("red_pixel_count")) 
val topFiveRedImagesDF = redPixelCountsDF
...
val redImageNames: Array[String] = topFiveRedImagesDF.select("annotation").as[String].collect()

// The number of red pixels in the five images with the most red pixels among the image group activities:
val redPixelAmounts: Array[Long] = topFiveRedImagesDF.select("red_pixel_count").as[Long].collect()
