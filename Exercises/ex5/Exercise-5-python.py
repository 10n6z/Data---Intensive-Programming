...
df_parquet: DataFrame = spark.read \
  .option("header", "true") \
  .option("inferSchema", "true") \
  .option("sep", "|") \
...
df_csv.write.csv(target_file_csv, header=True, sep="|")
...
df_new_rows.write.csv(target_file_csv, mode="append", header=True, sep="|")

# Append the new rows to Parquet storage:
df_new_rows.write.parquet(target_file_parquet, mode="append")

# COMMAND ----------

# Read the merged data from the CSV files to check that the new rows have been stored
df_new_csv: DataFrame = spark.read.csv(target_file_csv, header=True, sep="|")
...
df_modified: DataFrame = df_new_rows \
  .withColumn("ID", F.regexp_replace("ID", "Z1", "Z2")) \
  ...
  .withColumn("Temperature_C", (F.col("Temperature_C") - 32) * 5 / 9) \
  .drop("Description")
...
df_modified.write.csv(target_file_csv, mode="append", header=True, sep="|")

# Append the new modified rows to Parquet storage:
df_modified.write.parquet(target_file_parquet, mode="append")
...
modified_csv_df: DataFrame = spark.read.csv(target_file_csv, header=True, sep="|")
...
deltatable.alias("target") \
    .merge(df_delta_update.alias("source"), "target.ID = source.ID") \
...
deltatable.update(
  condition=F.col("Temperature_C").isNull(),
  set={"Temperature_C": F.round((F.col("Temperature_F") - 32) / 9 * 5, 1)}
)
