...
object Ex3Main extends App {
    ...
    // - Tasks 7-8 are similar basic tasks for textual data but using Datasets instead.
    //
    ...
    // The Parquet data contains the schema, so there is no need to infer or explicitly define the it.
    val procemDF: DataFrame = ???
    ...
    //
    // First row in wordCountRDD: word: 'logic.', count: 3
    // The most common 7-letter word that starts with 's': science (appears 93 times)



    printTaskLine(7)
    // Task 7 - Handling text data with Datasets
    //
    ...
    //     - Count the total number of non-empty lines in the article collection.
    //     - Pick the first 8 lines from the created Dataset and print them out.
    //
    // Part 2:
    //
    // - Do task 5 again, but this time using the higher level `Dataset` instead of the low level `RDD`.
    ...
    //
    // Hint: if you did the tasks 4 and 5, this task should be quite easy.
    ...
    printTaskLine(8)
    // Task 8 - Counting word occurrences with Dataset
    //
    // Do task 6 again, but this time using the higher level `Dataset` instead of the low level `RDD`.
    // Also, this time you should use the given case class `WordCount` in the resulting Dataset
    // instead of `(String, Int)` tuple like in task 6 with the RDD.
    // (the case class is defined at the beginning of this file)
    //
    ...
    //
    // - Using the created Dataset, find what is the most common 7-letter word that starts with an `s`,
    //   and what is its count in the collection.
    ...
    //       but give the result as an instance of the case class.
    //
    // In this task there are multiple ways that you can achieve the asked results.
    // You are free to choose whatever process you prefer, as long as the starting point is the `wordsDataset` from task 7,
