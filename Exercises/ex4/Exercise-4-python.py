...
dataDF: DataFrame = dataRDD.toDF(StructType([StructField("X", DoubleType(), False), StructField("label", DoubleType(), False)]))
...
lr: LinearRegression = LinearRegression(featuresCol="features", labelCol="label", maxIter=10, regParam=0.0)
...
x_df: DataFrame = spark.createDataFrame([(-2.8,), (3.14,), (9.9,), (123.45,)],['X'])
Task4: DataFrame = lrModel.transform(vectorAssembler.transform(x_df)).drop("features")
...
salesDF: DataFrame = spark.read \
  .format("csv") \
  .option("header", "true") \
  .option("sep", ";") \
  .option("inferSchema", "true") \
  .load("abfss://shared@tunics320f2024gen2.dfs.core.windows.net/exercises/ex4/static/superstore_sales.csv")

bestDaysDF: DataFrame = salesDF.groupBy("orderDate").agg((F.sum(F.col("productCount") * F.col("productPrice"))).alias("totalSales")).orderBy(F.desc("totalSales")).limit(8)
bestDaysDF.show()
...
myStreamingIdentifier: str = "long_pham"
...
salesStreamingDF: DataFrame = spark.readStream.format("csv").option("header", "false").option("sep", ";").schema(salesDF.schema).load(myStreamingFolder)
...
bestDaysStreamingDF: DataFrame = salesStreamingDF.groupBy("orderDate").agg((F.sum(F.col("productCount") * F.col("productPrice"))).alias("totalPrice"), F.sum("productCount").alias("totalCount")).orderBy(F.desc("totalPrice"))
...
def copyFiles(streamingQueryName: str, targetFolder: str) -> None:
    ...
    waitAfterFirstCopy: int = 30
    normalTimeInterval: int = 30
    postLoopWaitTime: int = 30
...
myStreamingQuery: StreamingQuery = bestDaysStreamingDF.writeStream.format("memory").queryName(streamQueryName).outputMode("complete").start()
