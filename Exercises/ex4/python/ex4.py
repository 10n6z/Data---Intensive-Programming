...
#   # Some resources that can help with the tasks in this exercise:
    ...
    dataDF: DataFrame = __MISSING__IMPLEMENTATION__

    # Split the data into training and testing datasets (roughly 80% for training, 20% for testing)
    trainingDF: DataFrame = __MISSING__IMPLEMENTATION__
    testDF: DataFrame = __MISSING__IMPLEMENTATION__
    ...
    vectorAssembler: VectorAssembler = __MISSING__IMPLEMENTATION__

    assembledTrainingDF: DataFrame = __MISSING__IMPLEMENTATION__

    assembledTrainingDF.printSchema()
    assembledTrainingDF.show(6)


    lr: LinearRegression = __MISSING__IMPLEMENTATION__
    ...
    lrModel: LinearRegressionModel = __MISSING__IMPLEMENTATION__
    ...
    testPredictions: DataFrame = __MISSING__IMPLEMENTATION__

    testPredictions.show(6)


    # all shown setters are optional, since they use the default values
    testEvaluator: RegressionEvaluator = __MISSING__IMPLEMENTATION__
    ...
    testError: float = __MISSING__IMPLEMENTATION__
    ...
    # The execution of the exercise code will continue once the plot window has been closed.
    ...
    salesDF: DataFrame = __MISSING__IMPLEMENTATION__

    bestDaysDF: DataFrame = __MISSING__IMPLEMENTATION__
    ...
    myStreamingFolder: str = "data/streaming"
    myStreamingPath: Path = Path(myStreamingFolder)
    # Ensure the folder exists when creating the streaming data frame
    if not myStreamingPath.exists():
        myStreamingPath.mkdir(parents=True)


    salesStreamingDF: DataFrame = __MISSING__IMPLEMENTATION__
    ...
    bestDaysStreamingDF: DataFrame = __MISSING__IMPLEMENTATION__
    ...
    waitAfterFirstCopy: int = 5
    normalTimeInterval: int = 3
    ...
    def copyFiles(targetFolder: str) -> None:
        streamingSource: str = "../../data/ex4/streaming"
        inputFileList: List[str] = glob.glob(f"{streamingSource}/*")

        for index, csvFile in enumerate(inputFileList):
            fileName: str = Path(csvFile).name
            inputFilePath: Path = Path(streamingSource) / fileName
            outputFilePath: Path = Path(targetFolder) / fileName
            ...
            print(f"Copied file {fileName} ({index + 1}/{len(inputFileList)}) to {outputFilePath} - waiting for {waitTime} seconds")
            time.sleep(waitTime)
    ...
    myStreamingQuery: StreamingQuery = __MISSING__IMPLEMENTATION__
    ...
    myStreamingQuery.awaitTermination(normalTimeInterval)
